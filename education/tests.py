...
class LessonTestCase(APITestCase):
    ...
    def test_lesson_retrieve(self):
        url = reverse("education:lessons_retrieve", args=(self.lesson.pk,))
    ...
    def test_lesson_update(self):
        url = reverse("education:lessons_update", args=(self.lesson.pk,))
        data = {
        ...
        data = response.json()
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertEqual(data.get("title"), "test postman updated")

    def test_lesson_delete(self):
        url = reverse("education:lessons_delete", args=(self.lesson.pk,))
...
class SubscriptionTestCase(APITestCase):
    ...
    def test_subscription_create(self):
        url = reverse("education:subscription")
        data = {"course_id": self.course.pk, "user": self.user}
        response = self.client.post(url, data=data)
        data = response.json()
        self.assertEqual(response.status_code, status.HTTP_200_OK)
